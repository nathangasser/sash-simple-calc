...
export default function CutSheetCalculator() {
  ...
  function handleExportCsv() {
    if (entries.length === 0) return;
    const csv = toCsv(entries, { showRoMin });
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `cut-sheet-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return (
    <div className="card">
      ...
      {entries.length > 0 && (
        <button type="button" className="export-link" onClick={handleExportCsv}>
          Export CSV
        </button>
      )}
